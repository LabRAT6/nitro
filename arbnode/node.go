...
func createNodeImpl(
	...
	if !config.ParentChainReader.Enable {
		return getNodeParentChainReaderDisabled(ctx, arbDb, stack, exec, txStreamer, blobReader, broadcastServer, broadcastClients, coordinator, maintenanceRunner, syncMonitor, configFetcher), nil
	}
	...
	inboxTracker, inboxReader, err := getInboxTrackerAndReader(arbDb, txStreamer, dapReaders, configFetcher, l1client, l1Reader, deployInfo, delayedBridge, sequencerInbox)
	if err != nil {
		return nil, err
	}
